...
- Use [Snowflake Marketplace](https://app.snowflake.com/marketplace/listing/GZTSZNUFX99I/dataiku-dataiku-for-enterprise-ai) to seamlessly create a Dataiku DSS Cloud trial
...
![img](assets/sf_i4_databases.png)
...
![img](assets/sf_i5_worksheets.png)
...
![img](assets/sf_i7_worksheet_rename.png)
...
![img](assets/sf_i8_query_history.png)
...
![img](assets/sf_i9_profile_accountadmin.png)
...
## Prepare Dataiku Trial Account Via Snowflake Marketplace
...
![4](assets/sf_4_query_education.png)

```sql
select * from RAW.JOB_POSTINGS limit 10;
```

![5](assets/sf_5_query_postings.png)
...
Let's start by switching to the Public schema as the Dataiku connection created from Snowflake Marketplace has permissions on that.
...
```
![6](assets/sf_6_query_postings_enriched.png)
...
> 
>**NOTE:** If you didn't setup AI Assistants from the Extensions menu in the earlier Snowflake Marketplace section do it now. 
...
1.To load the table, select the connection that was just created for us from `Snowflake Marketplace`. In the Table section select `Get Tables List`. Dataiku will warn you that this may be long list but we can OK this.
...
> that was created when we set up through Snowflake Marketplace.
> 
...
When using Dataiku's SaaS option from Snowflake Marketplace the setup is done for us automatically and we checked that in our earlier lab where we set up the AI Services. If for any reason you skipped that step earlier then return to your browser tab with `Dataiku Launchpad` open (if you have shut this just go to [Launchpad](https://launchpad-dku.app.dataiku.io/) and check that `Snowpark` is enabled under the `Extensions` 
...
- How to create a Dataiku trial account through [Snowflake Marketplace](https://app.snowflake.com/marketplace/listing/GZTSZNUFX99I/dataiku-dataiku-for-enterprise-ai)
